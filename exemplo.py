...
contator = 0 # Conta a quantidade de cachorros
...
while True:
    ...
    try:
        peso = int(input('Quantos quilogramas tem seu cachorro?\n' 
        '>>'))
        if 0 <= peso < 5:
            subtotal *= 1.8
        elif 5 <= peso < 12:
    ...
    # valor de desconto de 10% para cada cachorro
        desconto = (total * 0.1) * (contator - 1)
        total = total - desconto
        print('O total do(s) {} cachorro(s) ficou:\n'
        'R${:.2f} (desconto de R${:.2f})'.format(contator, total , desconto))
        break